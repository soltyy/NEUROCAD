...
Перевести панель NeuroCad на более аккуратный Claude-style layout без архитектурного дрейфа: улучшить иерархию сообщений, собрать input area в единый visual container, убрать визуальное обрезание user prompt в чате и устранить self-contradiction, когда LLM генерирует код с blocked tokens вроде `import`.

**Rolling Plan (старт)**
```
1. NC-DEV-UI-005A      / Developer / Claude-style panel layout refresh                    / planned
2. NC-DEV-CORE-013A    / Developer / Prompt-exec consistency for blocked tokens            / planned
3. NC-PM-REVIEW-005A   / PM        / Review UI refresh against release constraints         / planned
```
